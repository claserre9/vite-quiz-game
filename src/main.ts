...
console.info(
    `Quiz Math v${__APP_VERSION__} — build ${__BUILD_COMMIT__} (${__BUILD_DATE__})`
);
...
const pageBase = new URL(import.meta.env.BASE_URL, window.location.origin)
    .pathname;
const normalizedPageBase = pageBase !== '/' ? pageBase.replace(/\/$/, '') : '';
